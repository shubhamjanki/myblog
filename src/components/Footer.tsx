...
const Footer = () => {
  const { state } = useCms();
  const { twitterUrl, githubUrl, linkedinUrl, discordUrl, blogTitle } = state.settings;

  const socialLinks = [
    { label: "Twitter / X", icon: <TwitterIcon />, url: twitterUrl },
    { label: "GitHub", icon: <GitHubIcon />, url: githubUrl },
    { label: "LinkedIn", icon: <LinkedInIcon />, url: linkedinUrl },
    { label: "Discord", icon: <DiscordIcon />, url: discordUrl },
  ];
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8 mb-12">
          {footerNav.map((section) => (
            <div key={section.title}>
              <h4 className="font-display font-semibold text-sm text-foreground mb-3">{section.title}</h4>
            ...
            {socialLinks.map((s) =>
              s.url ? (
                ...
                <span
                  key={s.label}
                  aria-label={s.label}
                  className="w-9 h-9 flex items-center justify-center rounded-full text-muted-foreground/40"
                >
                  {s.icon}
                </span>
              )
